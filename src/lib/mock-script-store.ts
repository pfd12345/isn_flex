...
// ─── Default Mock Responses (seed data) ──────────────────────

const DEFAULT_RESPONSES: Record<string, string> = {
  _default: `Thank you for your message. I'm the ISN assistant and I'm here to help you progress through this workflow stage.
...
let currentScript: Record<string, string> = { ...DEFAULT_RESPONSES };

export function getMockScript(): Record<string, string> {
  return { ...currentScript };
}

export function updateMockScript(script: Record<string, string>): void {
  currentScript = { ...script };
}

export function updateMockResponse(stageId: string, response: string): void {
  currentScript[stageId] = response;
}

export function getDefaultMockScript(): Record<string, string> {
  return { ...DEFAULT_RESPONSES };
}

export function getMockScriptAsYaml(): string {
  return yaml.dump(currentScript, {
    lineWidth: -1,
    noRefs: true,
    sortKeys: false,
  });
...
export function parseMockScriptYaml(yamlString: string): Record<string, string> {
  const parsed = yaml.load(yamlString);

  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Invalid YAML: expected an object with stage IDs as keys');
  }

  const script: Record<string, string> = {};
  for (const [key, value] of Object.entries(parsed as Record<string, unknown>)) {
    if (typeof value !== 'string') {
      throw new Error(`Invalid value for key "${key}": expected a string`);
    }
    script[key] = value;
  }
